...
class Register(Resource):
    ...
    def post(self):
        ...
        db.sessions.insert_one(
            {"ip": request.remote_addr, "access_token": access_token, "refresh_token": refresh_token, "email": email})

        # New user created
        user = {'email': email, 'password': hashed_password, "date_created": date_created, "food_banks": [],
                "restaurants": []}
...
class GetSessions(Resource):
    def get(self):
        return json_util.dumps(db.sessions.find())
...
class GetSessionsForUser(Resource):
    ...
    def get(self):
        ...
        sessions = db.sessions.find({'email': get_jwt_identity()})
        return json_util.dumps(sessions)
